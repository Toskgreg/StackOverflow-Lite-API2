...
class AnswerAPI(MethodView):
    ...
    def post(current_user, question_id):
        ...
        return jsonify({'message': validate_answer(data)}), 406
    @staticmethod
    def put(current_user, question_id, answer_id):
        ...
        if question.qauthor == qauthor:
            data = request.get_json()
            print(data['status'])
            if data['status'] == 'accepted' or data['status'] == 'not accepted':
            ...
            return jsonify({'message': 'The status can only be in 3 states,' +
                'answered, accepted and not accepted'}),406
        query1 = database.fetch_by_param('answers', 'id', answer_id)
        if query1 is None:
            abort(404)
